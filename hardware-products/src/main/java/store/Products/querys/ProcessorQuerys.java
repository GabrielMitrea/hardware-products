...
//            "ON pr.categoryId=c.categoryId";
    ...
    public final static String GET_PROCESSOR1_SQL="select p.productName, pr.core, pr.frequency, pr.socket,pr.series, pr.manufacturingTechnology, p.productPrice\n" +
            "from proiect_java.processor pr inner join proiect_java.product p\n" +
            "on pr.productId=p.productId";

    
    public final static String ADD_PROCESSOR_SQL="insert into processor(processorId,core,frequency,socket,series,manufacturingTechnology,productId) values (null,?,?,?,?,?,?)";
