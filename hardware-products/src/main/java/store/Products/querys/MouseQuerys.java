...
public class MouseQuerys {
    ...
    public final static String GET_MOUSE1_SQL="select c.categoryName, p.productName, m.mouseType, m.maxResolution, m.interfacee, p.productPrice\n" +
            "from proiect_java.mouse m inner join proiect_java.product p\n" +
            "on m.productId=p.productId\n" +
            "inner join proiect_java.category c\n" +
            "on m.categoryId=c.categoryId";

    public final static String ADD_MOUSE_SQL="insert into mouse(mouseId,categoryId,mouseType,maxResolution,interfacee,productId) values (null,?,?,?,?,?)";
